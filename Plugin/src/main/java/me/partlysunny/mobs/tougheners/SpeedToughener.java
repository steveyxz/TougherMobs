...
public class SpeedToughener implements IMobToughener {
    ...
    public void toughen(Mob mob, ConfigurationSection config) {
        ...
        Optional<ConfigurationSection> speedSection = Util.getOptional(config, id(), ConfigurationSection.class);
        if (movementAttribute != null && speedSection.isPresent()) {
            ConfigurationSection speedConfig = speedSection.get();
            AttributeModifier.Operation operation;
            try {
                operation = AttributeModifier.Operation.valueOf(Util.getOrDefault(speedConfig, "operation", "MULTIPLY_SCALAR_1", String.class));
            ...
            Double value = Util.getOrDefault(speedConfig, "value", 0.0d, Double.class);
